...
const carRentalWithExpenseInput =
  "帮我添加一个从8月1日到8月9日的租车预订信息，并添加费用为13267DKK";
...
const carRentalWithExpenseResult = parseLocalItinerary(carRentalWithExpenseInput);
const carRentalWithExpenseReservation = carRentalWithExpenseResult?.reservations?.[0];
const carRentalWithExpense = carRentalWithExpenseResult?.expenses?.[0];
if ((carRentalWithExpenseResult?.reservations ?? []).length !== 1) {
  throw new Error("car rental with inline expense should still create one reservation draft");
}
if ((carRentalWithExpenseResult?.expenses ?? []).length !== 1) {
  throw new Error("car rental with inline expense should create one expense draft");
}
if (
  carRentalWithExpenseReservation?.reservation_type !== "car" ||
  carRentalWithExpenseReservation.title?.includes("13267") ||
  carRentalWithExpenseReservation.location_name?.includes("13267") ||
  carRentalWithExpenseReservation?.starts_at !== "2026-08-01T09:00:00" ||
  carRentalWithExpenseReservation?.ends_at !== "2026-08-09T18:00:00"
) {
  throw new Error("car rental reservation should parse cleanly without inline cost text");
}
if (
  carRentalWithExpense?.category !== "car" ||
  carRentalWithExpense?.original_amount !== 13267 ||
  carRentalWithExpense?.original_currency !== "DKK" ||
  carRentalWithExpense?.expense_date !== "2026-08-01" ||
  carRentalWithExpense?.end_date !== "2026-08-09"
) {
...
const tmbRoutePlanResult = parseLocalItinerary(tmbRoutePlanInput);
const tmbRouteEvents = tmbRoutePlanResult?.events ?? [];
if (tmbRouteEvents.length !== 2) {
  throw new Error(`TMB route plan should create 2 route events, got ${tmbRouteEvents.length}`);
}
if (
  tmbRouteEvents[0]?.title !== "TMB Day 1：Les Houches → Les Contamines" ||
  tmbRouteEvents[0]?.day_date !== "2026-08-14" ||
  tmbRouteEvents[0]?.planned_start !== "2026-08-14T09:00:00" ||
  tmbRouteEvents[0]?.planned_end !== "2026-08-14T17:00:00" ||
  tmbRouteEvents[0]?.location_name !== "Les Houches → Les Contamines" ||
  tmbRouteEvents[0]?.event_type !== "activity" ||
  tmbRouteEvents[0]?.is_estimated_time !== true
) {
  throw new Error("TMB route event should preserve date, estimated time, type, and route location");
}
if (
  !tmbRouteEvents[0]?.description?.includes("距离：18 km") ||
  !tmbRouteEvents[0]?.description?.includes("• Bellevue") ||
  !tmbRouteEvents[0]?.description?.includes("国家：法国")
) {
