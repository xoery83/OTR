...
import { renderMotionStorySkeleton } from "@/lib/memory-shots/motion-story-renderer";
...
async function requireJourneyAccess(
  ...
  if (!data) {
    throw new HttpError("You must be a journey member to render Motion Story.", 403);
  }
}
...
export async function POST(
  ...
  try {
    ...
    const result = await renderMotionStorySkeleton({
      supabase,
      memoryShotId,
    });
    ...
    return jsonError(
      errorMessage(error, "Could not render Motion Story."),
