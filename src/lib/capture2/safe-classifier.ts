...
function extractAmount(text: string) {
  const match =
    text.match(/(?:¥|￥|€|\$|£)?\s*(\d+(?:[.,]\d+)?)\s*(?:欧元|欧|美元|美金|英镑|日元|人民币|元|纽币|新西兰元|澳元|丹麦克朗|eur|usd|gbp|jpy|cny|rmb|nzd|aud|dkk)?/i) ??
    null;
  if (match?.[1]) return match[1].replace(",", ".");

  const chineseMatch = text.match(
    /((?:零|一|二|两|三|四|五|六|七|八|九|十|百|千|万)+(?:点(?:零|一|二|两|三|四|五|六|七|八|九)+)?)\s*(?:欧元|欧|美元|美金|英镑|日元|人民币|元|纽币|新西兰元|澳元|丹麦克朗)/i,
  );
...
function cleanExpenseTitle(text: string) {
  return (
    text
      ...
      .replace(/(?:¥|￥|€|\$|£)?\s*\d+(?:[.,]\d+)?\s*(?:欧元|欧|美元|美金|英镑|日元|人民币|元|纽币|新西兰元|澳元|丹麦克朗|eur|usd|gbp|jpy|cny|rmb|nzd|aud|dkk)?/gi, "")
      .replace(/(?:一共|总共|总计)/g, "")
      .replace(/(?:零|一|二|两|三|四|五|六|七|八|九|十|百|千|万)+(?:点(?:零|一|二|两|三|四|五|六|七|八|九)+)?\s*(?:欧元|欧|美元|美金|英镑|日元|人民币|元|纽币|新西兰元|澳元|丹麦克朗)/gi, "")
      .trim() || "费用支出"
  );
}
